...
docker compose --env-file "%ROOT%\.env" -f "%ROOT%\docker\docker-compose.dev.yml" -p tts-harness up -d postgres minio minio-init prefect-server
if errorlevel 1 goto :fail
...
echo [4/5] Opening visible service windows...
...
echo.
echo Started successfully in debug mode.
echo Service windows stay visible for logs.
echo.
